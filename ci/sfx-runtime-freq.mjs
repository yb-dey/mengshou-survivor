...
const LEN2SFX = {
  ...
  4851: 'SFX_UI',          // 0.11 —— 【第 53 轮修】母版 L12396 现为 `SFX_UI: len = 0.11`（0.11*44100=4851），
                           //   而本表还写着 0.12(5292) ⇒ 云端实测 **4851 触发 334 次被判"未知长度"**。
                           //   绊线按设计生效了：**表跟着母版漂了，红的是表、不是游戏**。只增不删，避免又造出新的未知。
  5292: 'SFX_UI(旧0.12)',  // 0.12（保留旧项：删掉反而可能把仍在用的长度变成新的「未知」）
  6174: 'SFX_KILL',        // 0.14
  7938: 'SFX_CARD',        // 0.18
  8820: 'SFX_HURT',        // 0.20
  ...
  44982: 'SFX_WIN',        // 1.02
  59535: 'SFX_LOSE',       // 1.35
  94500: '(jingle 胜利)'    // 2.143 —— 【第 53 轮修】母版 L8496 `win: refSmp=94500`（L12631 注释同值）。
                           //   钩子按 `>100k 判 BGM`，2.143s 的 jingle 只有 94,500 < 100k ⇒ 落进 SFX 侧且"未知"；
                           //   而失败 jingle 110,250 > 100k 走 BGM 分支 —— 两条 jingle 的分类口径本来就不一致（已记档）。
};
