...
const ALLOW = [
  ...
  // 【第 122 轮·台账同步】原有一条 `11px / sel ? "使用中" : "点选" / count 1` 的例外登记，
  //   已被 v1.211a **抬到 15px** ⇒ 该条目从台账里**删除**（不是失效：源码里那个站点仍在，只是不再 <12px）。
  //   删除的依据同样是运行时实测：这枚状态签是大厅上**必须动手操作**的短词里最小的一条（1.04mm）。
  //   ⚠ 若将来有人把它改回小号，本门禁会以"未登记"重新拦下 —— 这正是它该做的。
