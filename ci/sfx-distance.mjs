...
const ALLOW = [
  { pair: ['sfx_gem', 'sfx_hit'],
    why: '⚠ **判据盲区**（8 带 0.077 / 20 带 0.392）。' +
         '**实测独立依据**（2026-09-21 `_qc/_probe-gem-hit.mjs` 用同一套度量复现，非手写断言）: ' +
         '① 结构维 `strongBands` = **8 vs 0**（gem 宽带明亮 8 个带过 -6dB，hit 集中于 0 个）—— 该维**悬殊**; ' +
         '② 半音栅格重心 = **1535Hz vs 1080Hz（差 6.09 半音）**; ' +
         '③ 走向: hit **下行**(Δ-1.51) / gem 平(Δ-0.12); ' +
         '④ peakBand 4 vs 3、atk90 10 vs 5ms(2×)。' +
