...
本内容包当前以**美术**为主轴。为覆盖「美术 + 音乐」双支柱，下一轮计划在独立命名空间扩展：

1. **森林主题 BGM 原型**：仓库虽有 29 音源集，但缺独立的「森林」生物群落主题曲。将在 `_content_pack/audio/` 生成一首短循环原型（明确标注为占位/原型，待替换为制作级音轨），命名 `cp_bgm_forest` 等，避免与现有 `bgm_*` 冲突。
2. **新 creature 行为音效**：为 Boss 的「召唤藤蔓」「范围践踏」与英雄的「星辉治疗」「净化」补充专属 SFX（如 `cp_sfx_summon` / `cp_sfx_heal`），复用项目既有渲染/接线管线。
