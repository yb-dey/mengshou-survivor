...
## 2.20 挑战修饰符系统（每周轮换 + 真机选择器 · 本轮新增）

> 无尽模式重玩多样性：8 个全局修饰符按「玩家增益 × 敌人增强」成对设计——玩家侧（元素大师·火 30%/迅捷之风 移速 20%/聚宝周 币×2/泉涌之愈 治疗+50%）+ 敌人侧（敌潮汹涌 生成+30%/迅敌来袭 敌速+15%/铁壁敌阵 敌 HP+25%/狂暴化 敌伤+20%）。**每周按 ISO 周数种子自动选中 1 玩家 + 1 敌人修饰符**（同周一致、跨周轮换、可复现），为无尽模式提供「本周玩点」。修饰符只改变量起点、不改变难度曲线本身（与 R37 校准兼容，成对出现总难度≈基线±10%）。

| 文件 | 内容 | 用途 |
|---|---|---|
| `data/challenge_mods.json` | 修饰符规格 | 8 修饰符（玩家 4/敌人 4），每项含 `side / tag / effects（结构化字段）/ icon / sfx / desc` + rotation（ISO 周种子轮换规则）+ balanceNote |
| `mod_chart.html` | 修饰符规格页 | 轮换规则 + 平衡口径 + 8 修饰符卡（数据驱动，零图片生成） |
...
## 2.23 存档字段契约（接入规格 + 真机导入导出演示 · 本轮新增）

> 主游戏接入时**必须持久化什么、字段从哪来、脏数据怎么容错**的字段契约（纯设计规格）。11 个字段全部来自既有系统：meta_upgrades（已购强化等级）/ achievements（成就 + 每日任务进度）/ heroes（已解锁英雄）/ economy（森灵币余额）/ endless（无尽最佳记录）+ 统计 + 三通道设置。**明确排除项**：局内升级进度（upgrades.json）与局内状态一律不入档（一局结束即失效），避免存档膨胀与跨版本语义漂移。

| 文件 | 内容 | 用途 |
|---|---|---|
| `data/save_spec.json` | 存档字段契约 | schema 11 字段（含 keyRef 指向真实表）+ excluded 排除项 + integrity 四条容错规则（白名单丢弃/数值夹紧/默认值补齐/跨日重置）+ migration 只升不降策略 |
