...
//   【第 83 轮复测，母版 v1.208；**第 87 轮补记**】同一份**干净资产集**连跑也会跑动，实测区间
//   **1107–1110 张 / 48.5–51.6 Mpx ≈ 185–197 MB @RGBA**（±3 张来自首帧预渲染时机差异；
//   v1.209 复测为 1110 张 / 51.6 Mpx）⇒ **落在区间内都算正常，别把跑动当成「变胖了」**；
//   上界 55 Mpx 仍有约 6% 余量。
//   ⚠ 同一版母版在「素材有滞留」时会**虚高破线**：实测 1131 张 / 55.7 Mpx（❌），其中 +24 张 /
//   +4.1 Mpx 是 3 个已从母版删除却仍躺在 `assets/` 里的陈旧 `hero_*.webp` 造成的**假增量**；
//   手工删掉后又量到 1110 张 / 51.6 Mpx（仍不是真值）。⇒ 预算门禁红了先核对一件事：
//   `assets/` 是否与 `dist/assets/` 逐文件相等（`build-minigame.mjs` 自第 83 轮起先清空再递归复制）。
